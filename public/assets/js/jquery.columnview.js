...
(function($){
  ...
  var commands = {  
    ...
    $.fn.columnview = function() {  
        if (typeof arguments[0] === 'string') {  
            ...
            createColumnView.apply(this, arguments);  
        }  

        return this;  
    };  

  function createColumnView(options){
  ...
  function setOpen(presetLink, presetType)
  {
    var arr = new Array();

      $("#linkerList-processed [data-slug]").each(function()
      { 
          if($(this).data('slug') == presetLink && $(this).data('linktype') == presetType)
          {
                $parent = $(this).parent();

                while(!$parent.hasClass('toplevel'))
                {
                  $a = $parent.find("a:first");
                  var slug = $a.data('slug');

                  if($.inArray(slug, arr) == -1)
                  {
                    arr.push(slug);
                  }
      ...
      function selectNode()
      {
          $('#linkerList [data-slug]').each(function()
          {
              if($(this).data('slug') == arr[_this.arrIndex])
              {
                  ...
                  submenu(container, $(this), w);

                  if(_this.arrIndex < arr.length)
                  {
                    _this.arrIndex ++;
                    window.setTimeout(selectNode, 100);
